...
class ModelExtensionPaymentpayflex extends Model {
	...
	public function getAuthorizationCode() {
		...
		$AuthURL = '';
		$Audience = '';
		if($this->config->get('payflex_test')) {
		...
		$AuthBody = json_decode('{
				"client_id":"'.$this->config->get('payflex_username').'",
				"client_secret":"'.$this->config->get('payflex_password').'",
				"audience":"'.$Audience.'",
	...
	public function getCheckoutUrl($order, $cart) {
		...
		$OrderBodyString .= '],
			"merchant": {
				"redirectConfirmUrl": "' . $this->url->link('extension/payment/payflex/success') . '&status=confirmed",
				"redirectCancelUrl": "'  . $this->url->link('extension/payment/payflex/cancel') . '&status=cancelled",
				"statusCallbackUrl": "'  . $this->url->link('extension/payment/payflex/status') . '&status=status"
			},
	...
	public function getOrders() {
		...
		$sql .= " WHERE o.payment_code = 'payflex' AND (`date_added` > DATE_SUB(NOW(), INTERVAL 24 HOUR)) AND (o.order_status_id = '0' OR o.order_status_id = ".$this->config->get('payflex_order_status_pending')." OR o.order_status_id = ".$this->config->get('payflex_order_status_failed').")";
		...
		$updated_order_response_body = json_decode($updated_order_response);
			if (empty($updated_order_response_body->Errors)){
					...
					if ($returnedorderstatus == "Approved"){
							$order_status_id = $this->config->get('payflex_order_status_success');
						  $this->model_checkout_order->addOrderHistory($orderidtocheck, $order_status_id, 'PayFlex: ' . $returnedorderid, true);
							...
							$order_status_id = $this->config->get('payflex_order_status_pending');
							...
							$order_status_id = $this->config->get('payflex_order_status_expired');
							...
							$order_status_id = $this->config->get('payflex_order_status_cancelled');
						...
						$order_status_id = $this->config->get('payflex_order_status_failed');
	...
	public function updateCronJobRunTime() {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE `code` = 'payflex' AND `key` = 'payflex_cron_job_last_run'");
		$this->db->query("INSERT INTO `" . DB_PREFIX . "setting` (`store_id`, `code`, `key`, `value`, `serialized`) VALUES (0, 'payflex', 'payflex_cron_job_last_run', NOW(), 0)");
	}
